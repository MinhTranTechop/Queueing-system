  ...
  const listNgay = [
    ...new Set(
      progression.map((item) =>
        moment(item.DateStart_Pr, "HH:mm:ss - DD/MM/YYYY").format("MM")
      )
    ),
  ];
      ...
      for( let i = 0 ;i< listNgay.length ; i++ ){
        var count = 0;
      var downcount = 0;
        const dateToCalculate = progression.map((item)=>( (moment(item.DateStart_Pr, "HH:mm:ss - DD/MM/YYYY").format("MM")) === listNgay[i] ))
